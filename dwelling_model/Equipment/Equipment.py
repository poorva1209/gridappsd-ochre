...
class Equipment:
    ...
    def __init__(self, start_time, time_res, name=None, zip_model=None, air_node='LIV', ext_time_res=None, **kwargs):
        """
        ...
        self.max_time_by_mode = {mode: ext_time_res for mode in self.modes}
    ...
    def update(self, voltage, schedule, ext_control_args):
        ...
        if self.is_electric and self.zip_data and self.electric_kw:
    ...
    def generate_results(self, verbosity, to_ext=False):
        # Saves results for DOOM and for the external controller. By default, saves the mode and powers
        if to_ext:
            return {}
        else:
            if verbosity >= 6:
    ...
    def calculate_mode_priority(self, *duty_cycles):
        ...
        if duty_cycles:
            ...
            self.max_time_by_mode = {mode: d * self.ext_time_res for mode, d in zip(self.modes, duty_cycles)}
        ...
        modes_with_time = [mode for mode in self.modes if self.ext_mode_counters[mode] < self.max_time_by_mode[mode]]
        ...
        if not len(modes_with_time):
            self.print('Warning: No available modes, keeping the current mode. '
    ...
    def run_zip(self, v, v0=1):
        # pf_inductive = True for inductive load, False for capacitive
        if v == v0:
    ...
    def simulate(self, schedule=None, duration=None, voltage=1, equip_from_ext=None, verbosity=7):
        ...
        print('Running {} Simulation for {}'.format(self.name, duration))
        all_results = []
