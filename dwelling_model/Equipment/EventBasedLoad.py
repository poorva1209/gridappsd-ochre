...
class EventBasedLoad(Equipment):
    ...
    def __init__(self, **kwargs):
        ...
        # TODO: can remove from memory if events are generated during initialization
        self.probabilities, self.event_data = self.import_probabilities(**kwargs)
        ...
        # generate all events
        self.event_schedule = self.generate_all_events(**kwargs)
    ...
    def end_event(self):
        ...
        if self.event_index == len(self.event_schedule):
            # no more events - reset to last event index and move start/end times to the of the simulation
            self.event_index -= 1
            self.event_schedule.loc[self.event_index, 'start_time'] = dt.datetime.max
            self.event_schedule.loc[self.event_index, 'end_time'] = dt.datetime.max
    ...
    def update_external_control(self, schedule, ext_control_args):
        ...
        if delay and self.mode == 'On':
            print('WARNING: Ignoring {} delay signal, event has already started.'.format(self.name))
            delay = False
...
class DailyLoad(EventBasedLoad):
    ...
    def __init__(self, max_power=1, default_duration=dt.timedelta(hours=1), **kwargs):
        ...
        if self.default_duration % kwargs['time_res'] != dt.timedelta(0):
            ...
            print('Warning: Changing default duration of {} ({}) to align with simulation time.'
                  'New duration: {}'.format(self.name, self.default_duration, new_duration))
    ...
    def generate_all_events(self, **kwargs):
        ...
        idx = self.probabilities['Density'].searchsorted(r)
        df_events = self.event_data.iloc[idx].reset_index(drop=True)
